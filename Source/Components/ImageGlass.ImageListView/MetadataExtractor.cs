...
namespace ImageGlass.ImageListView
{
    ...
    internal class MetadataExtractor
    {
        ...
        {
            // Never use exceptions when you can use TryParse
            DateTime val;
            if (DateTime.TryParseExact(value, "yyyy:MM:dd HH:mm:ss", CultureInfo.InvariantCulture, DateTimeStyles.None, out val))
                return val;
            // For some reason, I have images with EXIF date values in "facebook" format.
        ...
        private void InitViaBmp(string path)
        {
            ...
            {
                if (Interpreter.IsImage(stream).IsSome)
                {
        ...
        private void InitViaWpf(BitmapMetadata data)
        {
            ...
            if (val != null)
            {
                var simpleRating = Convert.ToUInt16(val);
        ...
        {
            return query
                .Select(q => getQuery(q, metadata).Get(_ => null, Prelude.Identity))
                .FirstOrDefault(value => value != null);
        }
        Either<Exception,object> getQuery(string q, BitmapMetadata metadata) {
            try {
                return Either<Exception,object>.Right( metadata.GetQuery(q) );
            } catch (Exception ex) {
                return Either<Exception, object>.Left(ex);
            }
        }
#endif
        /// <summary>
