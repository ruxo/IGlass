...
namespace ImageGlass.Library.Image
{
    public static class ImageInfo
    {
        ...
        static readonly Dictionary<string,ImageFormat> extensionMapper = new Dictionary<string, ImageFormat>(StringComparer.OrdinalIgnoreCase){
            { "bmp", ImageFormat.Bmp },
            { "emf", ImageFormat.Emf },
        ...
        public static void SaveImage(System.Drawing.Image pic, string filename) {
            Contract.Requires(pic != null);
            Contract.Requires(!string.IsNullOrEmpty(filename));
            Option<string>
                .From(() => Path.GetExtension(filename))
                .Map(s => s.Substring(1))
                .Chain(extensionMapper.Get)
                .Do(format => pic.Save(filename, format));
        }
