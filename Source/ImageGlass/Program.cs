...
namespace ImageGlass {
    static class Program {
        static void SetupEnvironment() {
            ...
            var nativeInclude = Environment.Is64BitProcess ? x64Path : x86Path;

            var currentPath = Environment.GetEnvironmentVariable("PATH");
            Environment.SetEnvironmentVariable("PATH", $"{currentPath};{nativeInclude}");
        }
        ...
        private static void SingleInstance_ArgumentsReceived(object sender, ArgumentsReceivedEventArgs e)
        {
            ...
            Action<String[]> updateForm = async arguments =>
            {
                formMain.WindowState = FormWindowState.Normal;
                await formMain.LoadFromParams(arguments);
            };
